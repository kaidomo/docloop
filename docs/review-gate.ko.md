...
이 도구가 **보증하지 않는 것** — 전부 upstream 동작이며 다운스트림에서 고치지 않는다(고치면
포트가 아니라 포크가 된다):

- **entailment**(태그가 그 인용에서 따라 나오는가) — 매 실행 출력으로 공시하며, 사람 표본 감사로 메운다.
- **`match_strength`의 원천 결속** — 감사기가 태그 산출물을 입력받지 않고 manifest에도 값이 없다.
- **같은 severity·태그 그룹 안의 순서** — receipt 순서와 대조하지 않는다.
- **경로는 basename만 비교** — 같은 파일명의 다른 디렉터리는 반증되지 않는다. 다만 내용 해시는
  양쪽 다 결속되므로 다른 내용을 밀어 넣을 수는 없다. 경로는 보증이 아니라 표시 정보다.

이 중 실행 시점에 공시되는 것은 첫 항목뿐이다. 나머지 셋은 이 저장소 테스트로 고정해 뒀으므로,
upstream이 그중 하나를 바꾸면 이 포트가 알아챈다.
