...
| lib/blind_lock.py | semantic-port | meta-learning-loop (prediction lock) | - | - | v0.7.0 port. Source is a skill, not a single upstream file: no baseline can be recorded and drift is undetectable here (disclosed, not silently absent) |
| lib/panel_review.sh | semantic-port | cross-functional-review | - | - | v0.7.0 port. Source is a skill, not a single upstream file: drift undetectable here (disclosed) |
